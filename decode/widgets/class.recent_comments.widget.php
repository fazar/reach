<?php
class DC_recent_comments_widget extends WP_Widget {
	function __construct() {
		$widget_ops = array(
			'classname' => 'dc_recent_comments_widget', 
			'description' => __( 'Your site&#8217;s most recent comments.', THEMENAME ) );
		parent::__construct('dc-recent-comments', 'Decode - '.__('Recent Comments', THEMENAME), $widget_ops);
		$this->alt_option_name = 'dc_recent_comments';
	...
	function flush_widget_cache() {
		wp_cache_delete('dc_recent_comments_widget', 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['dc_recent_comments_widget']) )
			delete_option('dc_recent_comments_widget');
	...
	function widget($args, $instance) {
		...
		$cache = array();
		if ( is_preview() ) {
			$cache = wp_cache_get('dc_recent_comments_widget', 'widget');
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		$output .= '<ul id="dc-recent-comments">';
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
				$output .=  '<li class="dc-recent-comments">' . 
		...
		if ( is_preview() ) {
			...
			wp_cache_set( 'dc_recent_comments_widget', $cache, 'widget' );
		}
	}
