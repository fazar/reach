<?php
	class DC_instagram_widget extends WP_Widget{
		function __construct(){
			$widget_ops = array(
				'classname' => 'dc_instagram_widget', 
				'description' => __( "Display recent photos from instagram account.", THEMENAME) );
			parent::__construct('dc-instagram-widget', 'Decode - ' . __('Instagram', THEMENAME), $widget_ops);
			$this->alt_option_name = 'dc_instagram';
		}
					...
					if(!empty($username)){
						...
						<ul class="clearfix"><?php
							foreach ($media_array as $item) {
								echo '<li>
										<div class="dc-instagram-widget">
										<img src="'. esc_url($item['thumbnail']['url']) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"/>
										<a href="'. esc_url( $item['link'] ) .'" target="_blank"><div class="instagram-likes"><span class="icon-heart"></span><p>'.$item['likes'].'</p></div></a>
										</div>
									</li>';
							}
							?>
