<?php
	class DC_header_main extends DC_parts_base{

		function __construct(){
			parent::__construct();
			add_action( 'wp_head', array( $this, 'display_favicon' ) );
			add_action( 'dc_logo', array( $this, 'display_logo' ), 10, 1 );
			add_action( 'dc_html_head', array( $this, 'display_html_head' ) );
			add_action( 'dc_menu', array( $this, 'display_menu' ), 10, 2);
		}

		function display_favicon(){
			$options = $this->options;
			if ( !empty( $options['dc_favicon'] ) ) {
				$url = $options['dc_favicon']['url'];
				if( $url != null)   {
				  $type = "image/x-icon";
				  if(strpos( $url, '.png' )) $type = "image/png";
				  if(strpos( $url, '.gif' )) $type = "image/gif";
				  $html = '<link rel="icon" href="'.$url.'" type="'.$type.'">';
				  echo apply_filters( 'dc_favicon_display', $html );
				}
			}
		}
		...
		function display_logo( $scheme ){	
			if ( empty( $this->options['dc_use_logo'] ) || 
				$this->options['dc_use_logo'] != '1' ||
				empty( $this->options['dc_logo']) ){
				...
				if($scheme == 'light'){
					echo '<a class="logo-light" href="'. get_home_url() . '">
								<img src="' . $this->options['dc_logo']['url'] . '" 
					...
					echo '<a class="logo-dark" href="'. get_home_url() . '">
								<img src="' . $this->options['dc_dark_logo']['url'] . '" 
									 alt="' .get_bloginfo('name'). '" />
							</a>';
				}
			}
