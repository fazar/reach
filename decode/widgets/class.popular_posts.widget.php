<?php
	class DC_popular_posts_widget extends WP_Widget {

		function __construct() {
			$widget_ops = array(
				'classname' => 'dc_popular_posts_widget', 
				'description' => __( "Your site&#8217;s most Popular Posts.", THEMENAME) );
			parent::__construct('dc-popular-posts', 'Decode - ' . __('Popular Posts', THEMENAME), $widget_ops);
			$this->alt_option_name = 'dc_popular_posts_entries';
		...
		function widget($args, $instance) {
			$cache = array();
			if ( ! is_preview() ) {
				$cache = wp_cache_get( 'dc_popular_posts_widget', 'widget' );
			}
			...
			<ul id="dc-popular-posts" class="<?php echo $style ?>">
			...
			if ( !is_preview() ) {
				...
				wp_cache_set( 'dc_popular_posts_widget', $cache, 'widget' );
			} else {
				ob_end_flush();
			}
		}
		...
		function update( $new_instance, $old_instance ) {
			...
			if ( isset($alloptions['dc_popular_posts_widget']) )
				delete_option('dc_popular_posts_widget');
			return $instance;
		}

		function flush_widget_cache() {
			wp_cache_delete('dc_popular_posts_widget', 'widget');
		}
